...
export const deleteUser = async (userDetails, FILE) => {
    ...
    if(!email || !password) return {message: "All fields are required to delete the user"};
    ...
    return (response.status === 200) ? {message: `${user.name} has been deleted successfully`} : {message: `${user.name}  has not been deleted`};
}